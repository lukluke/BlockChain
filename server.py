...
# def new_transaction():
#     # addr = request.form.get('recipient_address','Missing recipient')
#     # amt = request.form.get('amount','Missing amount')
#     # values = request.get_json()
#     # addr = values.get('recipient_address')
...
#         response = {'message': 'Transaction will be added to Block ','recipient_address': values['recipient_address']}
...
def new_transaction():
    ...
    if float(usable_balance1(myWallet.identity)) < float(values['amount']):
...
def register_node():
    ...
    # retrieve nodes list
    node_list = requests.get('http://' + node + ":" + com_port + '/get_nodes')
...
@app.route('/confirmed_balance', methods=['POST','GET'])
...
@app.route('/unconfirm_balance', methods=['POST','GET'])
...
@app.route('/usable_balance', methods=['POST','GET'])
def usable_balance():
    values = request.form
    required = ['address']
    if not all(k in values for k in required):
        return 'Missing values', 400
    address = values.get('address')
    return str(blockchain.Current_balance(address) - blockchain.Holded_balance(address)), 200
...
def charge_transactions_fee():
    ...
    fee = Transaction(myWallet.identity, 'admin', values['fee'])
...
def usable_balance1(address):
    return str(blockchain.Current_balance(address) - blockchain.Holded_balance(address))
